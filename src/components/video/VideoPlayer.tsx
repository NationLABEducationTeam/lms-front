import { FC, useEffect, useRef, useState, forwardRef, ForwardedRef } from 'react';
...
const VideoPlayer = forwardRef<HTMLVideoElement, VideoPlayerProps>(
  ({ src, title, onTimemark }, ref) => {
    ...
    useEffect(() => {
      ...
      video.current.addEventListener('timeupdate', () => setCurrentTime(video.current!.currentTime));
      video.current.addEventListener('loadedmetadata', () => setDuration(video.current!.duration));
      video.current.addEventListener('volumechange', () => {
        setVolume(video.current!.volume);
        setIsMuted(video.current!.muted);
      });
      video.current.addEventListener('ratechange', () => setPlaybackRate(video.current!.playbackRate));
    ...
    const handleVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      video.current.muted = newVolume === 0;
    };
    ...
    // 타임마크 버튼 컴포넌트
    const TimemarkButton = () => (
      <button
        onClick={onTimemark}
        className="absolute top-4 right-4 px-3 py-2 bg-blue-600/90 hover:bg-blue-700/90 text-white rounded-lg backdrop-blur-sm flex items-center gap-2 transition-colors z-50"
      >
        <Clock className="w-4 h-4" />
        <span className="text-sm font-medium">타임마크</span>
      </button>
    );
